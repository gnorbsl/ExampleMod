...
const alarmEffect = newEffect(20, e => {
  Draw.color(Color.yellow, Color.red, e.fin()); //color goes from white to light gray
  Lines.stroke(e.fout() * 3); //line thickness goes from 3 to 0
  Lines.circle(e.x, e.y, e.fin() * 200); //draw a circle whose radius goes from 0 to 100
});

//create the block type
const alarm = extendContent(Block, "alarm-block", {
...
/*  buildConfiguration(tile, table){
  ...
  configured(tile, value){
    ...
    if(tile.entity.cons.valid()){
      ...
      Effects.effect(alarmEffect, tile)
